...
DevelopVersionString=`grep -E 'version.develop.*=' A_Category_MYL.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' A_Category_MYL.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' A_Category_MYL.podspec`
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' A_Category_MYL.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" A_Category_MYL.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' A_Category_MYL.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" A_Category_MYL.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' A_Category_MYL.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" A_Category_MYL.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' A_Category_MYL.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' A_Category_MYL.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" A_Category_MYL.podspec
...
pod repo push ${Repo} A_Category_MYL.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
